...
void threadWork(int id, int noOfThreads, int chunk, int totalRange, data *p);
...
int main() {
    ...
    for (int i = 0; i < m; ++i) {
        threads.push_back(thread(threadWork, i, m, c, N, &d[i]));
    }
    for (int i = 0; i < m; ++i) {
        threads[i].join();
    }
    ...
    cout << endl << "****** Static Allocation Method ******" << endl << endl;
    ...
    cout << endl << endl << "SAM Time: " << duration.count() << " ns" << endl;
...
void threadWork(int id, int noOfThreads, int chunk, int totalRange, data *p) {   
    for (int i = (chunk * id) + 1; i <= totalRange; i += (noOfThreads * chunk)) {
        p->ranges.push_back(i);
        for (int j = i; j < (i + chunk); ++j) {
            if (isSelfDivide(j)) {
                p->values.push_back(j);
            }
        }
    }
}